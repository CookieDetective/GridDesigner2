...
class SceneMode(BaseMode):
    ...
    def build(self, app, parent):
        ...
        drop_row.pack(fill=tk.X, padx=x_pad, pady=(0, 6))

        box = tk.LabelFrame(parent, text="Scene (Animation Connections)")
        box.pack(fill=tk.BOTH, expand=True, padx=x_pad, pady=(0, 8))
        ...
        row4 = tk.Frame(box)
        row4.pack(fill=tk.X, padx=x_pad, pady=(0, 6))
        ...
        app._scene_frame_listbox.pack(fill=tk.BOTH, expand=True, padx=x_pad, pady=(0, 6))
        app._scene_frame_listbox.bind("<<ListboxSelect>>", app.on_scene_frame_select)

        group_row = tk.Frame(frame_group_box)
        group_row.pack(fill=tk.X, padx=x_pad, pady=(0, 6))
        ...
        play_row.pack(fill=tk.X, padx=x_pad, pady=(0, 6))
        tk.Button(play_row, text="Play", command=app.play_scene).pack(side=tk.LEFT)
